...
n_values <- c(30, 50, 100, 200, 400, 1000)
...
# (LARGE) Conifdence Interval Simulation
...
# Create values of n (amount of gt data used)
n_values <- c(50, 200, 500, 1000, 2500, 5000)
...
for (n in n_values) {
  ...
  for (run in 1:num_runs) {
    ...
    for (setting in names(mp_dfs)) {
      ...
      # Naive OLS (replace mp points with ground truth at gt_ind)
      mp_naive_df <- mp_df
      mp_naive_df[gt_ind, ] <- gt_df
      ols_naive <- lm(y ~ lon + lat + dist, data = mp_naive_df)
      ci_naive <- confint(ols_naive)
...
# Store results
save(coverage_results, width_results, file = "ci_results_2d_large.R")
```
